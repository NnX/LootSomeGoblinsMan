...
public class PlayerScript : MonoBehaviour
{
    ...
    void MoveAndRotate()
    {
        if(moveVertical != 0)
        {
            playerBody.MovePosition(transform.position + transform.forward * (moveVertical * playerSpeed));
        }
    ...
    void Jump()
    {
        if(Input.GetKeyDown(KeyCode.Space))
        {
            ...
            {
                canJump = false;
                playerBody.MovePosition(transform.position + transform.up * (jumpForce * playerSpeed ));
